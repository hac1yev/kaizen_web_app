...
class InkisafController extends Controller
{
    ...
    public function all(){
        ...
        ->where('posts.status','1')->where('category_id','3')->orderBy('posts.created_at','DESC')->get();
    ...
    public function ferdi(){
        ...
        ->select('posts.id','posts.description','posts.content','posts.image','posts.view','posts.status','posts.category_id as category_id','categories.title as cat_title','posts.title as post_title','posts.created_at','posts.reading_time')
        ->where('posts.status','1')->where('category_id','3')->orderBy('posts.created_at','DESC')->get()->toArray();
        ...
        ->where('posts.status','1')->where('category_id','3')->orderBy('posts.created_at','DESC')->skip(11)->take(10)->get();
